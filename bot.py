...
async def autoapprove(client: app, message: Message):
    ...
    if message.from_user.mention is not None and message.chat.title is not None:
        await client.send_message(
                chat_id=user.id,
                text="**Hello {mention}!\nWelcome To {title}\n\nDo /start to know how to use me**".format(
    ...
    try:
        ...
        if m.chat.type == enums.ChatType.PRIVATE:
            keyboard = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("➕ Add me to your Chat ➕", url="https://t.me/Requestbhai_bot?startgroup")
                    ]
                ]
            )
            add_user(m.from_user.id)
            await m.reply_text("**Hello {}!\nI'm an auto approve Join Requests Bot.\nI can approve users in Groups/Channels.Add me to your chat and promote me to admin with add members permission.\n\n__Powered By : @OCTOPUSORIGNAL**".format(m.from_user.mention), reply_markup=keyboard)
    
        elif m.chat.type == enums.ChatType.GROUP or enums.ChatType.SUPERGROUP:
            keyboar = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("Start me PM", url="https://t.me/Requestbhai_bot?start=start")
                    ]
                ]
            )
            add_group(m.chat.id)
...
async def chk(_, cb : CallbackQuery):
    try:
        ...
        if cb.message.chat.type == enums.ChatType.PRIVATE:
            ...
            await cb.message.edit("**Hello {}!\nI'm an auto approve Join Request Bot.\nI can approve users in Groups/Channels.Add me to your chat and promote me to admin with add members permission.\n\n__Powered By : @OCTOPUSORIGNAL**".format(cb.from_user.mention), reply_markup=keyboard, disable_web_page_preview=True)
